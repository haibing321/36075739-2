...
var util_combo2array=combo2array;
var util_combo=combo;
var util_compact2array=compact2array;
var util_compact=compact;
;var DICT_RAW={
"a":"",
"ā":"吖锕錒",
"á":"嗄",
...
"bò,bǒ": "簸",
"bó,bù": "簿",
};function buildPinyinCache(d){var h,u={};for(var py in d){h=d[py];for(var i=0,c,l=h.length;i<l;i++){c=h.charCodeAt(i);if(!u[c])u[c]=py;else u[c]+=","+py}}return u}var D=buildPinyinCache(DICT_RAW);function A(t){for(var i=1;i<arguments.length;i++){var s=arguments[i];if(s)for(var k in s)if(s.hasOwnProperty(k))t[k]=s[k]}return t}var SN=0,ST=1,ST2=2,ST3=5,SI=3,SF=4,DO={style:ST,heteronym:false},IN="b,p,m,f,d,t,n,l,g,k,h,j,q,x,r,zh,ch,sh,z,c,s".split(","),RP=new RegExp("(["+Object.keys(PHONETIC_SYMBOL).join("")+"])","g"),RT=/([aeoiuvnm])([0-4])$/;function I(p){for(var i=0;i<IN.length;i++)if(p.indexOf(IN[i])===0)return IN[i];return""}function TF(py,s){switch(s){case SI:return I(py);case SF:var fl=py.charAt(0);if(PHONETIC_SYMBOL.hasOwnProperty(fl))fl=PHONETIC_SYMBOL[fl].charAt(0);return fl;case SN:return py.replace(RP,function(m,r){return PHONETIC_SYMBOL[r].replace(RT,"")});case ST3:return py.replace(RP,function(m,r){return PHONETIC_SYMBOL[r]});case ST2:var t="",p=py.replace(RP,function(m,r){t=PHONETIC_SYMBOL[r].replace(RT,"");return PHONETIC_SYMBOL[r].replace(RT,"")});return p+t;default:return py}}window.pinyin=function(h,o){if(typeof h!=="string")return[];o=A({},DO,o||{});var ps=[],nh="";for(var i=0,w,c,l=h.length;i<l;i++){w=h[i];c=w.charCodeAt(0);if(D[c]){if(nh.length>0){ps.push([nh]);nh=""}var s=D[c].split(",");!o.heteronym?ps.push([TF(s[0],o.style)]):function(){var ca={},r=[];for(var j=0;j<s.length;j++){var p=TF(s[j],o.style);if(!ca[p]){ca[p]=p;r.push(p)}}ps.push(r)}()}else nh+=w}if(nh.length>0)ps.push([nh]);return ps};window.pinyin.STYLE_NORMAL=SN;window.pinyin.STYLE_TONE=ST;window.pinyin.STYLE_TONE2=ST2;window.pinyin.STYLE_TO3NE=ST3;window.pinyin.STYLE_INITIALS=SI;window.pinyin.STYLE_FIRST_LETTER=SF})();